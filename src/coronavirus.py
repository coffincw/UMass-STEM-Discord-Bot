...
async def coronavirus(ctx):
    ...
    if len(argument) < 1:
        ...
        for state in sorted(data, key=lambda state: state['case'], reverse=True): # iterate through the state blocks sorted by case number
            population = -1
            comp = [value for key, value in us_areas.items() if str(state['state']).strip() in value]
            if len(comp) > 0:
                population = int(comp[0][1])
                population = round((state['death']/population) * 100, 4)
            
            pop_percentage = ' (' + str(population) + '%)' if population != -1 else ''
        ...
        try:
            ...
            comp = [value for key, value in us_areas.items() if state in value]
            if len(comp) > 0:
                population = int(comp[0][1])
        ...
        for block in data:
            if str(block['state']).strip() == state:
                found = True
                pop_percentage = ' ({:,.5f}%)'.format((block['death']/population) * 100) if population != -1 else ''
                description = 'Cases: ' + '{:,d}'.format(block['case']) + pop_percentage
